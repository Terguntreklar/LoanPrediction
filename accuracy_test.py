...
def divide_columns_by_max(dataframe): #Data scaling [0-50]
    newFrame = []
...
def feature_selection_tests(dataset, number_of_features,test_train_subsets):
    ...
    for model in Feature_Models: #apply RFE to get feature importance
        for i in range(0,1):
            model[1] = RFE(model[1],n_features_to_select=number_of_features)
            model[1] = model[1].fit(test_train_subsets[i][0],test_train_subsets[i][2])
            model[2] += model[1].ranking_
    data_sets_feature_selected = [] #this will hold different feature sets
    for feature_model in Feature_Models: #cut the original dataset 
    ...
    test_data = [] #this holds testing and training datasets
    for feature_dataset in data_sets_feature_selected: #create testing and training datasets
        arr = feature_dataset
        x = arr[:,0:-1]
        y = arr[:,-1]
        a = list(train_test_split(x,y,test_size=0.2))
        test_data.append(a)
    for testing_sub_dataset in test_data: #finally, calculate accuracy
        for model in models:
